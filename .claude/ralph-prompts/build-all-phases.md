...
You are building R360 Flow, a multi-tenant workflow automation platform. This prompt drives a Ralph Wiggum loop that iterates through all 6 phases, running plan-then-build for each.
...
1. Read the phase spec at `specs/Phase{N}.md`
...
1. Read the phase spec at `specs/Phase{N}.md` — focus on the completion checklist / acceptance criteria
...
When `currentPhase > 6` or `completedPhases` contains all 6 phases:
